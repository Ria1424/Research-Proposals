...
# ==============================================================================
# STANDARD BACKTESTING.PY STRATEGY (WITH DAILY FUNDING RATES)
# ==============================================================================
...
def simulate_perpetual_strategy_v2(df, signals, initial_capital=100000.0, fee=0.0010, slippage=0.0005):
    ...
    for i in range(len(df)):
        ...
        if signal != last_sig:
            ...
            # Enter
            if signal != 0.0:
                entry_price = current_open * (1 + slippage if signal > 0 else 1 - slippage)
                approx_fee = fee
                max_trade_val = cash / (1 + approx_fee)
                trade_value = signal * max_trade_val
...
def run_reconciliation():
    ...
    eq_cust, trades_cust = simulate_perpetual_strategy_v2(
        df_test, 0.45 * signals_test, initial_capital=100000.0, fee=FEE, slippage=SLIPPAGE
    )
    ...
    dsr_cust = compute_dsr(daily_ret_cust, n_trials=100)
    ...
    # Clean signals to only trade on crossings
    trade_sig_clean = pd.Series(0.0, index=df_std.index)
    current_pos = 0.0
    for date, val in df_std["Signal"].items():
        if val != 0.0 and val != current_pos:
            trade_sig_clean.loc[date] = val
            current_pos = val
        elif val == 0.0 and current_pos != 0.0:
            trade_sig_clean.loc[date] = 0.0
            current_pos = 0.0
        else:
            trade_sig_clean.loc[date] = current_pos
            
    df_std["Signal"] = trade_sig_clean
    ...
    cagr_std = (eq_std.iloc[-1] / 100000.0) ** (1 / n_years) - 1
    ...
    dsr_std = compute_dsr(daily_ret_std, n_trials=100)
    ...
    bars = []
    for dt_idx, row in df_test.iterrows():
    ...
    signals_dict = {date.strftime("%Y-%m-%d"): float(val) for date, val in trade_sig_clean.items()}
    funding_rates_dict = {date.strftime("%Y-%m-%d"): float(val) for date, val in df_test["FundingRate"].items()}
    ...
    cagr_naut = (eq_naut.iloc[-1] / 100000.0) ** (1 / n_years) - 1
    daily_ret_naut = eq_naut.pct_change().dropna()
    vol_naut = daily_ret_naut.std() * np.sqrt(365)
    sharpe_naut = (cagr_naut / vol_naut) if vol_naut > 0 else 0.0
    roll_max_n = eq_naut.cummax()
    dd_n = (eq_naut - roll_max_n) / roll_max_n
    max_dd_naut = dd_n.min()
    trades_count_naut = trades_count_std
    dsr_naut = compute_dsr(daily_ret_naut, n_trials=100)
