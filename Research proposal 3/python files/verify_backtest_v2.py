...
class NautilusValidationStrategy(NautilusStrategy):
    ...
    def on_bar(self, bar):
        ...
        # FIX: mark-to-market equity = cash balance + unrealised P&L
        cash_equity = 100000.0
        if account:
            ...
            if callable(bal):
                try:
                    cash_equity = float(bal(usdt))
                except Exception:
                    cash_equity = float(bal())
            else:
                cash_equity = float(bal)
                
        # Unrealised P&L: open position value at current close
        unrealised_pnl = current_size * float(bar.close)
        equity = cash_equity + unrealised_pnl
        ...
        # FIX: fractional sizing — use 0.001 precision instead of int() truncation
        SIZE_PRECISION = 0.001
        if sig != self.last_signal:
            target_units = (equity * 0.45) / float(bar.close)
            target_size = round(sig * target_units / SIZE_PRECISION) * SIZE_PRECISION
            
            size_diff = round((target_size - current_size) / SIZE_PRECISION) * SIZE_PRECISION
...
class StandardStrategy(Strategy):
    ...
    def next(self):
        ...
        # FIX: fractional sizing — use equity fraction (0.45) for backtesting.py
        SIZE_EQUITY_FRAC = 0.45
        if sig != self.last_sig:
            if sig == 0.0:
                self.position.close()
            elif sig > 0:
                self.position.close()
                self.buy(size=0.45 * 0.01)
            elif sig < 0:
                self.position.close()
                self.sell(size=0.45 * 0.01)
            self.last_sig = sig
...
def run_reconciliation():
    ...
    bt_std = Backtest(df_std, StandardStrategy, cash=100000, commission=lambda size, price: abs(size) * price * COMMISSION, margin=0.01)
    res_std = bt_std.run()
    
    eq_std = (res_std["_equity_curve"]["Equity"] / 100000.0).reindex(df_test.index).ffill()
    ...
    eq_naut_df = eq_naut_df.drop_duplicates(subset="Date", keep="last")
    eq_naut_df = eq_naut_df.set_index("Date")["Equity"]
    # Reindex to test period dates and forward-fill; then backward-fill leading NaN
    eq_naut = eq_naut_df.reindex(df_test.index, method="ffill").bfill()
    # Normalize to start at 1.0 (same scale as eq_std which is equity/100000)
    # eq_cust is in raw dollar units — normalize all three to start at 1.0
    eq_cust_norm = pd.Series(eq_cust, index=df_test.index)
    eq_cust_norm = eq_cust_norm / eq_cust_norm.iloc[0]
    eq_std_norm  = eq_std / float(eq_std.iloc[0]) if float(eq_std.iloc[0]) != 0 else eq_std
    # eq_naut may be in raw dollar units — normalize
    naut_start = float(eq_naut.iloc[0]) if len(eq_naut) > 0 and float(eq_naut.iloc[0]) != 0 else 100000.0
    eq_naut_norm = eq_naut / naut_start
    # Replace series used for metrics
    eq_cust = eq_cust_norm
    eq_std  = eq_std_norm
    eq_naut = eq_naut_norm
    ...
    dsr_naut = m_naut["dsr"]
    # FIX: measure Nautilus trade count from actual filled orders — do NOT copy from Standard
    try:
        filled_orders = [o for o in engine.cache.orders() if hasattr(o, 'status') and str(o.status) in ("FILLED", "OrderStatus.FILLED")]
        trades_count_naut = len(filled_orders) if filled_orders else "N/A"
    except Exception:
        trades_count_naut = "N/A"
