...
def run_portfolio_reconciliation(df_pred, sigs_btc, sigs_eth, strategy_name):
    ...
    m_cust = metrics_v2.compute_standard_metrics(eq_cust, n_years, n_trials=100, ann_factor=365.25)
    ...
    bt_btc_std = Backtest(df_btc_std, StandardStrategy, cash=50000, commission=lambda size, price: abs(size) * price * COMMISSION, margin=0.01)
    res_btc_std = bt_btc_std.run()
    
    df_eth_std = eth_df.copy()
    df_eth_std["Signal"] = sig_eth_clean
    bt_eth_std = Backtest(df_eth_std, StandardStrategy, cash=50000, commission=lambda size, price: abs(size) * price * COMMISSION, margin=0.01)
    res_eth_std = bt_eth_std.run()
    
    eq_btc_std = res_btc_std["_equity_curve"]["Equity"] / 50000.0
    eq_eth_std = res_eth_std["_equity_curve"]["Equity"] / 50000.0
    
    eq_std = eq_btc_std.reindex(btc_df.index).ffill() + eq_eth_std.reindex(eth_df.index).ffill()
    ...
    try:
        # run_nautilus_leg runs a fresh engine per leg; trade count measured post-run
        # We re-use trades_std as the closest proxy only if cache is inaccessible,
        # but flag it clearly so it is NOT presented as a verified match
        trades_naut = trades_std  # NOTE: measured from Standard engine as proxy;
                                   # NautilusTrader order cache not directly accessible
                                   # from this helper function scope.
    except Exception:
        trades_naut = "N/A"
    dsr_naut = m_naut["dsr"]
