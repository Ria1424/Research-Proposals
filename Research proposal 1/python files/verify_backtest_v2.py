...
class NautilusValidationStrategy(NautilusStrategy):
    ...
    def on_bar(self, bar):
        ...
        # account.balance_total is cash-only (staircase) — add open position value
...
def run_reconciliation():
    ...
    # Load model signals
    sig_path = RESULTS_DIR / "signals_btc_v2.csv"
    if not sig_path.exists():
        raise FileNotFoundError(f"Missing signals file {sig_path}. Run proposal1_main_v2.py first.")
    ...
    for name in ["BTC", "ETH"]:
        ...
        # Sizing at 45% of equity (0.45 * margin) with margin=0.01 (100x leverage limit)
        ...
        # FIX: measure Nautilus trade count from actual filled orders
        # Do NOT copy from Custom engine — counts must be independently measured
        try:
            filled_orders = [o for o in engine.cache.orders() if hasattr(o, 'status') and str(o.status) in ("FILLED", "OrderStatus.FILLED")]
            trades_naut = len(filled_orders) if filled_orders else "N/A"
        except Exception:
            trades_naut = "N/A"
        dsr_naut = m_naut["dsr"]
        ...
        aligned_df = pd.DataFrame({
            ...
            "Std": eq_std.reindex(eq_cust_scaled.index).ffill(),
            "Nautilus": eq_naut.reindex(eq_cust_scaled.index).ffill()
        }).dropna()
        ...
        eq_compare = pd.DataFrame({
            "Custom": eq_cust_scaled,
            "Standard": eq_std.reindex(eq_cust_scaled.index).ffill(),
