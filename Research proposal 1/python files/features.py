...
NOTE (OHLCV-only run):
  BTCUSDT_daily.csv and ETHUSDT_daily.csv do NOT contain a `funding_rate`
  column, so `fr` defaults to a zero-series in build_features().  All
  funding-rate derived features (fr_raw, fr_5d_avg, fr_zscore_*, fr_roc_*,
  fr_high_flag, fr_low_flag) will be constant zero and are dropped by
  LightGBM's NaN / zero-variance handling during training.

  Similarly, `taker_buy_volume` is absent, so tbr_* features are all-NaN
  and are dropped during model training.

  The current reproducible pipeline is therefore an OHLCV-only ML
...
def build_features(ohlcv: pd.DataFrame, name: str = "ASSET") -> pd.DataFrame:
    ...
    # upper shadow = distance from candle top (high) to the top of the body
    # top of body = max(open, close)  [FIX: was incorrectly using min via clip(upper=)]
    body_top = pd.concat([o, c], axis=1).max(axis=1)
    feats["upper_shadow_pct"] = (h - body_top) / c
