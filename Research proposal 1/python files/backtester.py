...
Key design guarantees:
  ...
  - Cost-aware: taker fee + slippage + daily funding rate applied to open positions.
  - Metrics output: CAGR, Vol, Sharpe, Sortino, Calmar, MaxDD, WinRate, ProfitFactor,
    NumTrades, Turnover — all gross AND net of costs.
...
class BacktestResult:
    ...
    equity_gross: pd.Series          # equity curve (gross)
...
class Backtester:
    ...
    def __init__(self, ohlcv: pd.DataFrame, cost_model: CostModel = None):
        """
        ohlcv: DataFrame with columns [open, high, low, close, volume, funding_rate]
        """
    ...
    def run(self, signal: pd.Series, label: str = "Strategy") -> BacktestResult:
        ...
        for i in range(n - 1):
            ...
            funding = ohlcv.get("funding_rate", pd.Series(0.0, index=ohlcv.index)).iloc[i + 1]
            ...
            if i + 1 < n:
                ...
                funding_cost = current_pos * funding  # positive fr = cost for longs
                gross_daily = daily_return
                net_daily = daily_return - funding_cost

                # Cost for position changes is applied at trade close (above)
                # For ongoing positions, only funding applies
    ...
    def _compute_metrics(self, result: BacktestResult):
        ...
        # ── CAGR ───────────────────────────────────────────────────────
        result.cagr_net = (eq_net.iloc[-1] ** (1 / n_years) - 1)
        result.cagr_gross = (eq_gross.iloc[-1] ** (1 / n_years) - 1)
        ...
        result.sharpe_net = excess.mean() / excess.std() * np.sqrt(self.DAYS_PER_YEAR) if excess.std() > 0 else 0
        result.sharpe_gross = (daily_ret_gross - rf_daily).mean() / (daily_ret_gross - rf_daily).std() * np.sqrt(self.DAYS_PER_YEAR)

        # ── Sortino ────────────────────────────────────────────────────
        downside = daily_ret_net[daily_ret_net < rf_daily]
        downside_std = downside.std() * np.sqrt(self.DAYS_PER_YEAR)
        result.sortino_net = excess.mean() * self.DAYS_PER_YEAR / downside_std if downside_std > 0 else 0
        ...
        # ── Calmar ─────────────────────────────────────────────────────
        result.calmar_net = result.cagr_net / abs(result.max_dd_net) if result.max_dd_net != 0 else 0
